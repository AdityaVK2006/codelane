  ...
  switch(status) {
    case 'Green':
      html = `<div class="${pulseClass}" style="
        ...
        box-shadow: 0 0 20px rgba(0,0,0,0.8), 0 0 25px #22c55e;
        cursor: pointer;
        position: relative;
        z-index: 1000;
      ">
    ...
    case 'Yellow':
      html = `<div class="${pulseClass}" style="
        ...
        border-radius: 50%; 
        border: ${borderWidth}px solid white;
        box-shadow: 0 0 20px rgba(0,0,0,0.8), 0 0 25px #eab308;
        cursor: pointer;
        position: relative;
        z-index: 1000;
      ">
    ...
    case 'Red':
      html = `<div class="${pulseClass}" style="
        ...
        border-radius: 50%; 
        border: ${borderWidth}px solid white;
        box-shadow: 0 0 20px rgba(0,0,0,0.8), 0 0 25px #ef4444;
        cursor: pointer;
        position: relative;
        z-index: 1000;
      ">
...
const createPedestrianIcon = (isSelected = false) => {
  ...
  const html = `<div style="
    ...
    border-radius: 50%; 
    border: ${borderWidth}px solid white;
    box-shadow: 0 0 15px rgba(0,0,0,0.6), 0 0 20px #3b82f6;
    display: flex;
    align-items: center;
  ...
  useEffect(() => {
    if (selectedId) {
      ...
      if (intersection && intersection.position) {
        map.setView(intersection.position, 14, {
          animate: true,
          duration: 1
        });
      }
...
// MapView component without road highlighting
const MapView = forwardRef(({ 
  ...
  // Bhubaneswar coordinates - Centered for spread out view
  const mapCenter = [20.3000, 85.8200];
  ...
  return (
    <>
          ...
          return (
            <div key={intersection.id}>
              ...
              {intersection.pose && (
                <Marker 
                  position={[
                    intersection.position[0] + 0.0015,
                    intersection.position[1] + 0.0015
                  ]}
              ...
              >
                <Popup>
                  <div className="p-2">
                    ...
                    <p className="text-xs text-gray-600 mt-1">
                      {intersection.vehicles} vehicles detected • Wait: {intersection.waitingTime}
                    </p>
                  </div>
                </Popup>
              </Marker>
        ...
        {events.filter(event => event.status !== 'resolved').map((event) => {
          ...
          return (
            <Marker
              key={event.id}
              position={[
                relatedIntersection.position[0] + 0.002,
                relatedIntersection.position[1] - 0.002
              ]}
              icon={L.divIcon({
                html: `<div style="
                  ...
                  box-shadow: 0 0 15px rgba(0,0,0,0.6), 0 0 20px ${eventColor};
                  display: flex;
                  align-items: center;
                  justify-content: center;
                  color: white;
                ...
                <div className="p-2 min-w-[260px]">
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <span className={`text-xs px-3 py-1 rounded font-semibold ${
                      event.severity === 'high' ? 'bg-red-100 text-red-800 border border-red-300' :
                      event.severity === 'medium' ? 'bg-yellow-100 text-yellow-800 border border-yellow-300' :
      ...
      <style jsx global>{`
        ...
        /* Enhanced marker shadows for better visibility */
        .traffic-light-marker, .pedestrian-marker, .congestion-marker, .event-marker {
