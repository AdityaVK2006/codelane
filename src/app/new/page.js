...
const Page = () => {
  ...
  // Reference to the MapView component to call its methods
  const mapRef = useRef();

  // Function to focus on a specific intersection
  const focusOnIntersection = (intersection) => {
    // Call the focus method on the MapView component if it exists
    if (mapRef.current && mapRef.current.focusOnIntersection) {
      mapRef.current.focusOnIntersection(intersection);
    }
    
    // Also update the selected intersection and show the popup
    setMapDetails({
      ...mapDetails,
  ...
  const handleSearch = (query) => {
    ...
    if (query.length > 0) {
      const results = intersections.filter(intersection =>
        intersection.name.toLowerCase().includes(query.toLowerCase())
      );
  ...
  const handleEmergencyOverride = () => {
    if (selectedIntersection) {
      ...
      const event = {
        ...
        time: new Date().toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'}),
        status: "active",
        description: "Emergency override activated - green light extended to 60s"
      };
  ...
  const handleAddEvent = () => {
    if (newEvent.title && newEvent.location) {
      const event = {
        ...
        time: newEvent.time || new Date().toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})
      };
        ...
        <div className="relative mb-6">
          <div className="flex items-center border border-gray-300 rounded-lg overflow-hidden">
            ...
            <input
              ...
              className="w-full py-2 pr-3 focus:outline-none bg-transparent"
          ...
          {showSearchResults && searchResults.length > 0 && (
            <div className="absolute z-10 w-full mt-1 bg-white border border-gray-200 rounded-lg shadow-lg max-h-60 overflow-y-auto">
              {searchResults.map((intersection) => (
                <div
                  key={intersection.id}
                  className="p-3 hover:bg-blue-50 cursor-pointer border-b border-gray-100 last:border-b-0"
                  onClick={() => selectSearchResult(intersection)}
                >
                  <div className="font-medium">{intersection.name}</div>
                  <div className="text-sm text-gray-500 flex justify-between">
                    <span>Congestion: {intersection.congestion}</span>
                    <span className={
                      intersection.light === "Green" ? "text-green-500" :
                      intersection.light === "Red" ? "text-red-500" :
                      "text-yellow-500"
                    }>
          ...
          {showSearchResults && searchResults.length === 0 && searchQuery && (
            <div className="absolute z-10 w-full mt-1 bg-white border border-gray-200 rounded-lg shadow-lg p-3">
              <div className="text-gray-500">No intersections found</div>
            </div>
          )}
        </div>
      ...
      <main className="relative h-full flex-[0.8] rounded-2xl overflow-hidden bg-gray-200">
        ...
        {mapDetails.showPopup && selectedIntersection && (
          <div className="absolute top-4 right-4 bg-white p-4 rounded-lg shadow-lg z-50 w-80">
            ...
            >
              {selectedIntersection.pose ? "Control Traffic Light" : "No Control Available"}
            </button>
          </div>
        )}
      </main>
          ...
          >
            <div className="p-4 flex flex-col h-full justify-between">
              ...
              <div className="flex items-center justify-between">
                ...
                <button
                    onClick={() => focusOnIntersection(intersection)}
                    className="px-2 py-1 bg-gray-100 text-gray-600 rounded text-sm hover:bg-gray-200 transition"
                    title="Focus on map"
                  >
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" />
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 11a3 3 0 11-6 0 3 3 极 0112 0z" />
                    </svg>
                  </button>
              </div>
            </div>
      ...
      {open && selectedIntersection && (
        <div className="fixed inset-0 bg-white z-[10000] flex flex-col">
          ...
          <div className="flex items-center justify-between p-6 border-b">
            <h2 className="text-2xl font-bold">{selectedIntersection.name} - Traffic Control</h2>
            <button
          ...
          <div className="flex flex-1 overflow-hidden">
            ...
            <div className="w-1/3 border-l p-6 overflow-y-auto scrollbar-hide">
              <div className="mb-6">
                <h3 className="text-lg font-semibold mb-3">Intersection Information</h3>
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-gray-50 p-3 rounded">
                    <p className="text-sm text-gray-600">Traffic Light</p>
                    <p className={
                      selectedIntersection.light === "Green" ? "text-green-500 font-semibold text-lg" :
                      selectedIntersection.light === "Red" ? "text-red-500 font-semibold text-lg" :
                  ...
                  <div className="bg-gray-50 p-3 rounded">
                    <p className="text-sm text-gray-600">Timer</p>
                    <p className="font-semibold text-lg">{selectedIntersection.lightTimer}s</p>
                  </div>
                  <div className="bg-gray-50 p-3 rounded">
                    <p className="text-sm text-gray-600">Congestion</p>
                    <p className={
                      selectedIntersection.congestion === "Low" ? "text-green-500 font-semibold text-lg" :
                      selectedIntersection.congestion === "Medium" ? "text-yellow-500 font-semibold text-lg" :
                  ...
                  <div className="bg-gray-50 p-3 rounded">
                    <p className="text-sm text-gray-600">Vehicles</p>
                    <p className="font-semibold text-lg">{selectedIntersection.vehicles}</p>
                  </div>
                  <div className="bg-gray-50 p-3 rounded col-span-2">
                    <p className="text-sm text-gray-600">Average Waiting Time</p>
                    <p className="font-semibold text-lg">{selectedIntersection.waitingTime}</p>
                  </div>
                </div>
              </div>
              
              <div className="mb-6">
                <h3 className="text-lg font-semibold mb-3">Traffic Light Control</h3>
                <div className="mb-4">
                  <label className="block text-sm font-medium mb-2">Set Light Duration (seconds)</label>
                  <div className="flex items-center">
                    ...
                    />
                    <span className="ml-3 font-semibold w-10">{manualTime}s</span>
                  </div>
                </div>
                ...
                <p className="text-xs text-gray-500 text-center">Sets green light for 60 seconds</p>
              </div>
              ...
              <div>
                ...
                <div className="space-y-2 max-h-40 overflow-y-auto">
                  {events
                    .filter(event => event.location.includes(selectedIntersection.name))
                    .slice(0, 3)
                    .map(event => (
        ...
        <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-white p-6 rounded-lg z-[9999] w-96 shadow-xl">
          ...
          <div className="mb-4">
            ...
            <input 
              ...
              className="w-full p-2 border rounded"
              placeholder={newEvent.type === "construction" ? "e.g., 09:00-16:00" : "e.g., 14:30"}
            />
          </div>
